...
excl1 = os.path.join(root_dir, "2月份员工资金变动流水.xls")   # 拼接excel表格地址
...
df3 = pd.read_excel(excl3, sheet_name='Sheet2')               # 张怡君名单
...
num1 = len(result5.index)     # 产互刷卡次数
num2 = len(result7.index)     # 非产互刷卡次数
result5['刷卡总次数'] = None
result7['刷卡总次数'] = None
result5.iloc[0, 19] = num1
result7.iloc[0, 19] = num2
...
result7.to_excel(new_wb, sheet_name='非产互刷卡次数', index=False)     # 保存result7的数据到非合同制主业 页
